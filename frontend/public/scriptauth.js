...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginForm) {
      loginForm.addEventListener("submit", async (e) => {
        ...
        try {
          // Envoie l'email et le mot de passe via axios
          const response = await api.post('/auth/login', { email, password });
  
          // La réponse de 'axios' est déjà un objet, pas besoin de .json()
          if (response.status === 200) {
            displayMessage('login-message', "Connexion réussie !", 'success');
            localStorage.setItem('token', response.data.token); // Token de la réponse
            window.location.href = './home.html';
          } else {
            displayMessage('login-message', response.data.message || "Erreur de connexion.", 'error');
          }
        } catch (error) {
          console.error("Erreur lors de la connexion :", error);
          const message = error.response?.data?.message || "Impossible de se connecter au serveur.";
          displayMessage('login-message', message, 'error');
        }
      });
    }
    // if (signupForm) {
